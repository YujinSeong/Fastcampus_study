...
#오토엔코더는 인풋을 작은차원으로 축소한 후 다시 풀어내는것, 재구축된 아웃풋은 인풋과 동일해야한다
#히든레이어는 인풋의 특징을 압축적으로 표현, 분류기 만들 때 그냥 데이터를 바로 쓰기보다는 오토 엔코더를 사용하여 입력시 성능이 향상된다
#이 코드는 오토인코더로 인해 softmax의 성능이 얼마나 향상되는지 보기위한 코드
#fine tuning(transfer learning) : 변수를 초기화한상태에서 뉴럴넷을 만드는 것은 학습 효과가 확실하지 않음
#다초기값을 random sampling을 사용하는 것이 아닌 다른사람들이 사용한 어떤 분류에 특화된 W, b를 가져와서 새로운 분류기를 가져와서 사용하는 것
#강아지나 고양이나 어쨌든 분류를 사용할때 어떤 특징을 가지고 하는데 강아지를 잘분류하면 고양이도 잘 분류할 가능성이 높다
#잘 동작하는 분류기의 W,b를 가져와서 다른 문제를 푸는게 좋당
