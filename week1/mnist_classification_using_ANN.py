...
# MNIST 데이터를 다운로드 한다.
from tensorflow.examples.tutorials.mnist import input_data #mnist는 많이 다루는 데이터로 텐서플로우에서 미리 전처리되어 tmp/data에 저장
mnist = input_data.read_data_sets("/tmp/data/", one_hot=True)

# 학습을 위한 하이퍼 파라미터들 (학습을 위해 사람이 미리 지정하는 값)
learning_rate = 0.001   #alpha
training_epochs = 30    # 학습횟수
batch_size = 256        # 배치개수, 한번 수행할때 전체 데이터를 256개씩 묶어서 gradient함
...
x = tf.placeholder(tf.float32, [None, input_size]) #none은 입력갯수를 임의로 받겠다는 의미 input은 784가 될것임
true_y = tf.placeholder(tf.float32, [None, output_size]) #아웃풋은 1x10

# ANN 모델 구현 (모듈함수로 구현)
def build_ANN(x):
    W1 = tf.Variable(tf.random_normal([input_size, hidden1_size])) #변수는 tf.Variable이라는 api로 선언 normal distribution에서 임의의 값을 하나 지정
...
loss = tf.reduce_mean(tf.nn.softmax_cross_entropy_with_logits(logits=predicted_value, labels=true_y)) # 실제와 가설의 차이
optimizer = tf.train.AdamOptimizer(learning_rate=learning_rate).minimize(loss) # optimizer로 adam사용

# 세션을 열고 그래프를 실행한다.
with tf.Session() as sess:
    # 변수들에 초기값을 할당한다.
    sess.run(tf.global_variables_initializer()) #노드에 사용하는 모든 변수(random 하게 sampling한 값들) 초기화

    # 지정된 횟수만큼 학습을 진행한다.
    for epoch in range(training_epochs):
        average_loss = 0.
        # 전체 배치를 불러온다.
        total_batch = int(mnist.train.num_examples/batch_size) #데이터는 5만개이지만 256개씩 batch로 묶어서 트레이닝 (배치단위 트레이닝)
